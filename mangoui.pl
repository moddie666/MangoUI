...
our $prog_name = "MangoUI.pl";
# Variables
our $userhome =  "/home/".getpwuid($>);                       # user home dir
our $default_path = $userhome."/.config/MangoHud/";           # "load" dialog default location
our $file_path = $userhome."/.config/MangoHud/MangoHud.conf"; # default file to load upon startup
our $icon_path = $userhome."/.local/share/icons";             # logo path
our $icon_file = $icon_path."/minimango.png";                 # logo, will be unpacked from base64 below
...
if (!( -e $icon_file) ) {
   if (!( -d $icon_path) ) {
         mkdir $icon_path; # or die;
      }
   write_file($icon_file, $decoded_image);
}
...
my $quit_action = Gtk3::AccelGroup->new;
$mw->add_accel_group($quit_action);
$mw->add_accelerator('activate', $quit_action, Gtk3::Gdk::KEY_q, ['control-mask'], 'visible');
$mw->signal_connect(key_press_event => sub {
    my ($widget, $event) = @_;
    if (($event->state & 'control-mask') && $event->keyval == Gtk3::Gdk::KEY_q) {
        print "Exit requested by CTRL-Q\n";
        Gtk3->main_quit;
        return 1;
    }
    return 0;
...
$mw->set_icon_from_file($icon_file);
